...
class CategoriesTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        PFQuery(className: "Category").findObjectsInBackground { (categories, error) in
            if let e = error {
                AlertHelper.showAlert(self, title: "Ocorreu um erro", message: e.localizedDescription, confirmText: "OK")
                return
            }
            
            if let categoryList = categories as? [Category] {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let categoryCell = cell as? CategoryTableViewCell {
            categoryCell.bind(categories[indexPath.row])
        }
